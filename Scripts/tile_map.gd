...
var board_size = 4 # size of board on x and y
enum Layers{hidden, revealed} # layers for if a card is hidden or revealed
var SOURCE_NUM = 0 # refers to atlus of tilemap
const hidden_tile_coords = Vector2(6,2)
const hidden_tile_alt = 1
var revealed_spots = []
var tile_pos_to_atlas_pos = {} # keep track of all card pos + map to pos in atlus
var score = 0
var turns_taken = 0
...
# chose what cards from atlus we want to use
# I'm gonna use the top row :D
func get_titles_to_use():
...
# go through each spot on board and place facedown card
func setup_board():
	var cards_to_use = get_titles_to_use()
	...
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_LEFT and event.is_pressed():
			...
			if current_tile_alt == 1 and revealed_spots.size() < 2:
				...
				# check if they revealed 2 cards total
...
func when_two_cards_revealed():
	...
	# adds 1 to the number of turns taken
	turns_taken += 1
	update_text()
